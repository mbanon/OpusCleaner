...
const categories = computed({
  get() {
    if (currentDataset.value === null) return null;

    if (modifiedCategories.value !== null) return modifiedCategories.value;
...
watch(
  ...
  function (current, old) {
    if (current?.name !== old?.name) modifiedCategories.value = null;
  },
...
<template>
  <Teleport to="body">
    <div
      ref="element"
      v-if="currentDataset !== null"
      class="popup"
      :style="position"
    >
      <header class="title">Set categories</header>
...
.popup {
  ...
  min-width: 220px;
  box-shadow: 2px 2px 8px rgba(0, 0, 0, 0.215);
  padding: 10px;
}
